 ...
 * @thorws exceptions
 */
export async function put(reqURL: string, body: object, returnJson = true, headers?: HeadersInit, option?: optionSettings) : Promise<any>
{
  //Validate config
  validateOptions(option);
  
  //Create options
  let options = assignConfig("PUT", headers, option);
